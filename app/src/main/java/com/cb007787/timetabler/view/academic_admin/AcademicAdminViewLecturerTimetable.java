...
public class AcademicAdminViewLecturerTimetable extends AppCompatActivity {

    private String usernameOfLecturer;
    private LectureService lectureService;
    private String token;
    private AuthReturn loggedInUser;
    private List<LectureShow> lecturesForLecturer = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_academic_admin_view_lecturer_timetable);
        ...
        if (intent != null) {
            //retrieve string assigned from the UserRecycler.
            usernameOfLecturer = intent.getStringExtra("lecturerUsername");
        }
        ...
        swipeRefreshLayout.setOnRefreshListener(this::getLecturesForLecturer);
        ...
        adapter = new LectureRecycler(this, lecturesForLecturer);
        LectureRecycler.setUserRole(loggedInUser.getRole());
        LectureRecycler.setIsAcademicAdminViewingLecturerTimetable(true); //disable edit and delete
        recyclerView.setLayoutManager(new LinearLayoutManager(this));
        recyclerView.setAdapter(adapter);

        toolbar.setTitle("Timetable - " + usernameOfLecturer);
    ...
    protected void onStart() {
        super.onStart();
        getLecturesForLecturer();
    }

    private void getLecturesForLecturer() {
        startDate = null;
        endDate = null;
        selectedDate.setText(null);
        progressIndicator.setVisibility(View.VISIBLE);
        lectureService.getLectureForLecturer(token, usernameOfLecturer).enqueue(new Callback<List<LectureShow>>() {
            @Override
            public void onResponse(@NonNull Call<List<LectureShow>> call, @NonNull Response<List<LectureShow>> response) {
                ...
                if (response.isSuccessful()) {
                    lecturesForLecturer = response.body();

                    adapter.setTheLectureList(lecturesForLecturer);

                    if (lecturesForLecturer.size() == 0) {
                        constructError("There are no lectures available for this lecturer. Schedule lectures for modules", true);
                    }
                } else {
                    try {
                        ...
                        e.printStackTrace();
                        constructError("We ran into an error while loading the lectures", false);
                    }
                }
            }
    ...
    private void filterTheLecturesForStartDateAndTime() {
        List<LectureShow> filteredList =
                this.lecturesForLecturer.stream().filter(
